"""
Libya PMM - East Region Municipality Food & NFI MEB Charts (Gap Linked)
Shows East municipalities (AlBayda, Benghazi, Derna, Ejdabia, AlKufra, Tobruk) with National
...
Usage:
    python scripts/06_Visualizations/charts/east_meb_charts.py <year> <month> [--type food|nfi]

Examples:
    python scripts/06_Visualizations/charts/east_meb_charts.py 2025 11
    python scripts/06_Visualizations/charts/east_meb_charts.py 2025 11 --type food
    python scripts/06_Visualizations/charts/east_meb_charts.py 2025 11 --type nfi
"""
...
def get_municipality_time_series(engine, target_date, region="East", months_back=12):
    """
    Query database for East municipality Full MEB and return pivoted DataFrame.
...
def create_east_meb_chart_gaplinked(data, meb_type="food_meb", year=None, month=None):
    """
    Create East region municipality Food or NFI MEB chart (gap-linked).
    ...
    print(f"Loading East municipality time series from database...")
    muni_dfs = get_municipality_time_series(engine, target_date, region="East", months_back=12)
    muni_df = muni_dfs[meb_type]

    if muni_df.empty:
        raise ValueError("No East municipality data returned.")
    ...
    chart_name = f"East_{meb_label}_MEB_{month_tag}.svg"
...
def main():
    print("=" * 70)
    print("EAST REGION - FOOD & NFI MEB CHARTS (GAP-LINKED)")
    print("=" * 70)

    # Get year and month from command line
    if len(sys.argv) < 3:
        print("❌ Usage: python scripts/06_Visualizations/charts/east_meb_charts.py <year> <month>")
        print("   Example: python scripts/06_Visualizations/charts/east_meb_charts.py 2025 11")
    ...
    try:
        ...
        if chart_type in ["both", "food"]:
            print("\n" + "-" * 70)
            print("Generating East Food MEB Chart (Gap-Linked)...")
            print("-" * 70)
            file = create_east_meb_chart_gaplinked(data, "food_meb", year, month)
            generated_files.append(file)

        if chart_type in ["both", "nfi"]:
            print("\n" + "-" * 70)
            print("Generating East NFI MEB Chart (Gap-Linked)...")
            print("-" * 70)
            file = create_east_meb_chart_gaplinked(data, "nfi_meb", year, month)
            generated_files.append(file)

        print("\n" + "=" * 70)
        print("✅ GAP-LINKED EAST CHART GENERATION COMPLETE")
        print("=" * 70)
